...
export const slotTokens = v.object({
  ...
  // --- motion sequences (interactions, option B) ---------------------------
  // The multi-step half of the interaction decision, settled 2026-08-20 in
  // favour of BOUNDED SEQUENCES over a real timeline. A part may carry an
  // ordered list of steps and one trigger; it may not carry a scroll-scrubbed
  // track, because that needs a sequencer running in every visitor's browser
  // and the published page still contains no client JavaScript on the motion
  // path. That boundary is the whole decision, and it is written here because
  // this validator is where it can be enforced rather than remembered.
  //
  // A sequence SUPERSEDES `motion` on the same part rather than composing with
  // it. Two animations on one element fight over `transform` and the last one
  // declared wins silently, which is the exact failure the one-motion-per-part
  // rule was written to avoid. The renderer ignores `motion` whenever a
  // sequence is present, and the panel says so.
  motionSequence: v.optional(
    v.object({
      /** What starts it. Three, and the missing fourth is the point: a CLICK
       *  trigger cannot be expressed in CSS for an arbitrary part, so offering
       *  it would mean shipping a listener to every published page. It is not
       *  in the vocabulary, so nobody can pick it and be quietly disappointed.
       *
       *  `view` is scroll-into-view, the same window the band's own reveal
       *  uses. `hover` runs under the pointer and is suppressed on a coarse
       *  pointer. `load` runs once when the page paints. */
      trigger: v.union(
        v.literal("view"),
        v.literal("hover"),
        v.literal("load"),
      ...
      steps: v.array(
        v.object({
          /** The same named motions a single part can already have, plus
           *  `pulse` — the one step that is emphasis rather than an entrance,
           *  and therefore the only one that makes a SECOND step worth having.
           *  No free durations, distances or easings here either: every
           *  emitted value still reads the site's own `--site-motion-*`. */
          motion: v.union(
            v.literal("rise"),
            v.literal("fade"),
            v.literal("zoom"),
          ...
          /** How long to wait after the previous step finishes, in the same
           *  bounded steps `motionDelayStep` uses. */
          delayStep: v.optional(v.number()),
        }),
      ),
    }),
  ),
...
/** Steps in one sequence. Four is a sequence; ten is a timeline, and a
 *  timeline is the option this decision rejected. The cap is enforced at
 *  sanitize so an import or an older build cannot smuggle a longer one in. */
export const SLOT_SEQUENCE_STEP_MAX = 4;

export const SLOT_SEQUENCE_TRIGGER_KEYS = ["view", "hover", "load"] as const;
export type SlotSequenceTrigger = (typeof SLOT_SEQUENCE_TRIGGER_KEYS)[number];

export const SLOT_SEQUENCE_MOTION_KEYS = [
  "rise",
  "fade",
  "zoom",
  "pulse",
] as const;
export type SlotSequenceMotion = (typeof SLOT_SEQUENCE_MOTION_KEYS)[number];
