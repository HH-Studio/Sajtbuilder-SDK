...
/**
 * Full machine-readable shape plus the cross-field invariants enforced by
 * `validateImportReport`. JSON Schema covers shape/bounds; the namespaced
 * invariant list makes referential/count/status rules part of the hashed
 * contract instead of undocumented validator behavior.
 */
    ...
    ...strictObject(
      {
        ...
        adapter: strictObject(
          {
            id: stableId,
            version: nonEmptyString(IMPORT_REPORT_LIMITS.version),
          },
          ["id", "version"],
        ),
        sourceInputs: {
          type: "array",
          minItems: 1,
          maxItems: IMPORT_REPORT_LIMITS.sourceInputs,
          items: strictObject(
            {
              id: stableId,
              kind: { enum: ["url", "file"] },
              locator: nonEmptyString(IMPORT_REPORT_LIMITS.locator),
              contentHash,
            },
            ["id", "kind", "locator"],
          ),
        },
        detectedPlatform: strictObject(
          {
            id: stableId,
            version: nonEmptyString(IMPORT_REPORT_LIMITS.version),
            confidence,
        ...
        requiredVersions: strictObject(
          {
            reportFormat: { const: IMPORT_REPORT_FORMAT_VERSION },
            portableSiteFormat: { const: PORTABLE_SITE_FORMAT_VERSION },
            cli: nonEmptyString(IMPORT_REPORT_LIMITS.version),
          },
          ["reportFormat", "portableSiteFormat", "cli"],
        ),
        evidence: {
          type: "array",
          maxItems: IMPORT_REPORT_LIMITS.evidence,
          items: strictObject(
            {
              id: stableId,
              kind: { enum: [...EVIDENCE_KINDS] },
              sourceInputId: stableId,
              locator: nonEmptyString(IMPORT_REPORT_LIMITS.locator),
              contentHash,
              excerpt: nonEmptyString(IMPORT_REPORT_LIMITS.excerpt),
            },
        ...
        items: {
          type: "array",
          maxItems: IMPORT_REPORT_LIMITS.items,
          items: {
            ...strictObject(
              {
                id: stableId,
                disposition: { enum: [...IMPORT_DISPOSITIONS] },
                reason: nonEmptyString(IMPORT_REPORT_LIMITS.reason),
                evidenceIds: {
                  type: "array",
                  minItems: 1,
                  maxItems: IMPORT_REPORT_LIMITS.evidenceIdsPerItem,
                  uniqueItems: true,
                  items: stableId,
                },
                target: strictObject(
                  {
                    kind: nonEmptyString(IMPORT_REPORT_LIMITS.id),
                    id: nonEmptyString(IMPORT_REPORT_LIMITS.locator),
                  },
                  ["kind", "id"],
                ),
                confidence,
            ...
            allOf: [
              {
                if: { properties: { disposition: { const: "ai_proposed" } } },
        ...
        summary: strictObject(
          {
            total: count,
            blocking: count,
            byDisposition: strictObject(
              Object.fromEntries(IMPORT_DISPOSITIONS.map((value) => [value, count])),
              IMPORT_DISPOSITIONS,
            ),
          },
          ["total", "blocking", "byDisposition"],
        ),
      },
      [
        "format",
