...
/**
 * The single source of truth for this template.
 *
 * `defineSite` is a typed identity helper from `@snabbsajt/site-kit`. The object
 * you write here IS a SnabbSajt portable site (`PortableSiteV1`): the same data
 * drives the Next.js preview in this repo AND the `.zip` you import into
 * SnabbSajt (`npm run build:snabbsajt`). Edit content here and both stay in sync.
 *
 * Rules that keep the round-trip clean:
 *  - Only use section `type`s and `variant`s that exist in the SnabbSajt
 *    registry (autocomplete + `npm run build:snabbsajt` enforce this).
 *  - `theme` uses tokens, never raw hex.
 *  - `order` is a fractional-indexing key ("a0", "a1", ...), unique per page.
 *  - Home page slug is "". Other slugs are language-agnostic ("about", ...).
 */
  ...
  sections: [
    // ---- Home ----
    {
      ...
      content: {
        ...
        subheadline:
          "We help small teams turn a good idea into a website, a brand, and a plan people actually act on.",
    ...
    },
    {
      ...
      content: {
        ...
        primaryCta: { label: "Get in touch", target: { kind: "page", pageSlug: "contact" } },
      },
    },
    ...
    // ---- About ----
    {
      ...
      content: {
        ...
        body: "We are a small studio in Stockholm. We work with founders and small teams who want work that looks considered and reads clearly. No jargon, no bloat, just the useful parts done well.",
      },
    },
    {
      pageTmpId: "about",
      ...
      content: {
        ...
        heading: "The team",
        members: [
          { name: "Alex Nord", role: "Design" },
          { name: "Sam Berg", role: "Strategy" },
          { name: "Robin Ek", role: "Build" },
        ],
      },
    },
